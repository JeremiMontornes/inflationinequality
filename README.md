...
- Calculate and visualize category- and household-level inflation
...
The package structures the workflow around reusable data objects. Loader and constructor functions create `cpi`, `hbs`, and `index_weights` objects, while calculation functions return structured `weights`, `price_indices`, `inflation`, `contributions`, and `household_inflation` objects that can be passed directly to plotting and validation helpers.
