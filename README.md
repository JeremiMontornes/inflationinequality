...
|:---|:---|:---|:---|:---|
| Price data source | Eurostat HICP via the hicp package, using the current ECOICOP v2 monthly index. | custom_cpi in calculate_inflation() / calculate_contributions(); load_cpi() date and level arguments. | Yes | The resolver currently refers to ECOICOP v2 datasets. |
| HBS source | Eurostat HBS via DBnomics. | custom_hbs in calculate_weights(), calculate_inflation(), and calculate_contributions(). | Yes | National-source pipelines can be added as country-specific custom data workflows. |
...
| HBS interpolation | No interpolation by default. | interpolated_hbs = TRUE. | Yes | Useful when HBS waves users want smoother weights over time. |
| Specific HBS wave | No single HBS wave is set by default. | specific_hbs_year. | Yes | Useful for sensitivity analysis or producing a fixed-wave methodology. |
| Missing CPI series | Missing CPI series are not estimated by default. | ensure_complete_cpi = TRUE. | Yes | Estimated CPI data use parent-category price movements. |
