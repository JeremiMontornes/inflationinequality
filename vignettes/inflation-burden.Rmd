...
- [Eurostat `icw_sr_10`](https://db.nomics.world/Eurostat/icw_sr_10): aggregate propensity to consume by income quintile, expressed as a percentage of disposable income.
- [Eurostat `hbs_exp_t133`](https://db.nomics.world/Eurostat/hbs_exp_t133): mean consumption expenditure by income quintile, available in PPS per household or PPS per adult equivalent.
...
## BLS-style substitution-bias table

The BLS compares a Lowe-formula research CPI (`R-CPI-I`) with a chained Törnqvist-formula research CPI (`R-C-CPI-I`). The difference between their annualized growth rates is interpreted as an upper-level substitution-bias measure.
...
- `lowe_growth`: annualized growth rate of the package's Lowe-style fixed-weight aggregation.
- `toernqvist_growth`: annualized growth rate of a chained Törnqvist aggregation computed with `hicp::toernqvist()`.
- `substitution_bias`: `lowe_growth - toernqvist_growth`, in percentage points.

This is not a perfect replication of the BLS table, because Eurostat HBS income-quintile expenditure shares are not available at the same monthly frequency as the U.S. Consumer Expenditure Survey inputs used by BLS. The package therefore uses annual category-specific weights and approximates the Törnqvist comparison with previous-year and current-year weights.
